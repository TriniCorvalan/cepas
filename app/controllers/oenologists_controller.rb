class OenologistsController < ApplicationController
  ...
  def index
    @oenologists = Oenologist.order("age ASC")
  end
  ...
  def create
    ...
    params[:oenologist][:job_ids].each do |job_id|
      if !job_id.empty?
        @oenologist.jobs << Job.find(job_id)
      end
    end
  ...
  private
    ...
    def oenologist_params
      params.require(:oenologist).permit(:name, :age, :nationality, :workplace, jobs_attributes: [:position])
    end
end
